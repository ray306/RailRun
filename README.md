...
默认先在 `config.json` 中保存 rail 文件地址：
...
当流程名不是 `.rail` 文件名且被 `[]` 包裹时，RailRun 会从 `config.json` 的 `rails_alias_and_path` 字段读取真实 rail 路径。这是推荐的默认调用方式。未使用 `[]` 的输入会按普通路径解析，仅建议临时调试或直接指定一次性文件时使用。
...
参考：`PROTOCOL.md` 与 `examples` 目录，自行或借助 AI 把任务流程写成 `rail` 文件。

你也可以用 `md`/`txt` 作为流程输入（`/RailRun {{md/txt 文件路径}}`），RailRun 会先尝试转写再执行。建议在生成后检查是否转写正确。

---

## 运行模式与网页 UI

RailRun 支持两种运行模式以适配不同的执行环境：

### 1. Base 模式（默认）
* **特点**：无后台常驻进程，零配置。CLI 本地直接读写 Session 状态文件完成状态机演进。
* **适用场景**：沙箱环境、无网络监听权限的容器、以及像 Claude 网页端等支持本地 Skill 但不便拉起守护进程的运行环境。

### 2. Daemon 模式（包含原生网页 UI）
* **特点**：基于标准库 HTTP 服务启动后台守护进程，原生搭载一个精致、美观的单文件图形化 **流程控制台（网页 UI）**。
* **网页 UI 地址**：`http://127.0.0.1:8799/`
* **网页 UI 核心功能**：
  - **会话总览**：实时列出所有 Session 的状态（进行中、已结束、需人工介入、已停止）。
  - **进度可视化**：选定会话后，直观展示该会话对应的原始控制流节点（CFG）以及详细的演进历史轨迹，并标记当前步骤游标。
  - **控制台调度**：在网页端一键点击“暂停流程”（转为人工介入）、“终止”会话，或选择历史中的任意一步“跳转回滚（Rewind）”。
* **如何启用**：
  - **临时启用**：在 CLI 初始化会话时增加 `--use-daemon` 参数：
    ```bash
    python next_step.py --procedure "[simple_todo_flow]" --use-daemon
    ```
  - **默认启用**：在配置文件 `config.json` 中添加 `"use_daemon": true`，以后每次运行即可默认自动使用守护进程模式：
    ```json
    {
      "rails_alias_and_path": {
        "simple_todo_flow": "examples/simple_todo_flow.rail"
